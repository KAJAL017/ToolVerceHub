...
<form id="tagForm" method="POST">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="name" id="name" value="{{ old('name', $blogTag->name) }}" required
        ...
        <div class="mb-4">
            ...
            <input type="text" name="slug" id="slug" value="{{ old('slug', $blogTag->slug) }}"
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" rows="3"
                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
                placeholder="Brief description of this tag">{{ old('description', $blogTag->description) }}</textarea>
            <p class="text-red-500 text-sm mt-1 hidden" id="error-description"></p>
        </div>
        
        <div class="mb-6">
            <label class="flex items-center cursor-pointer">
                <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured', $blogTag->is_featured) ? 'checked' : '' }}
                    class="w-5 h-5 text-yellow-500 border-gray-300 rounded focus:ring-yellow-500 focus:ring-2">
                <span class="ml-3 text-sm font-semibold text-gray-700">
                    <i class="fas fa-star text-yellow-500 mr-1"></i>
                    Mark as Featured Tag
                </span>
            </label>
            <p class="text-xs text-gray-500 mt-1 ml-8">Featured tags will appear in the blog sidebar</p>
        </div>
        ...
        <div class="flex items-center space-x-3">
            ...
            <a href="{{ route('admin.blog-tags.index') }}" class="px-6 py-3 bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold rounded-lg transition-colors">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const demoData = [
        ...
        },
        {
            name: 'Quick Tip',
            slug: 'quick-tip',
            description: 'Short tips and tricks'
        },
        {
            name: 'How To',
            slug: 'how-to',
    ...
    form.addEventListener('submit', function(e) {
        ...
        fetch('{{ route("admin.blog-tags.update", $blogTag) }}', {
            method: 'POST',
            body: formData,
            headers: {
        ...
        .then(data => {
            if (data.success) {
                window.location.href = '{{ route("admin.blog-tags.index") }}?success=' + encodeURIComponent(data.message || 'Tag updated successfully!');
            } else if (data.errors) {
