...
@section('page-subtitle', 'Manage blog tags')
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
    ...
    <div class="p-6 border-b border-gray-200">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h3 class="text-lg font-bold text-gray-800">All Tags</h3>
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('admin.blog-tags.create') }}" class="inline-flex items-center px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white font-semibold rounded-lg transition-colors">
    ...
    <div class="overflow-x-auto">
        <table class="w-full" id="tagsTable">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($tags as $tag)
                <tr class="hover:bg-gray-50 transition-colors" data-name="{{ strtolower($tag->name) }}" data-slug="{{ strtolower($tag->slug) }}" data-description="{{ strtolower($tag->description ?? '') }}">
                ...
                <tr>
                    <td colspan="5" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center justify-center">
                            ...
                            <p class="text-gray-400 text-sm mt-1 mb-4">Create your first tag to organize your blogs</p>
                            <a href="{{ route('admin.blog-tags.create') }}" class="inline-flex items-center px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white font-semibold rounded-lg transition-colors">
                                <i class="fas fa-plus mr-2"></i>
                                Create Tag
                            </a>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function handleDelete(tagId, tagName) {
        ...
        if (confirmed) {
            ...
            fetch(`{{ url('admin/blog-tags') }}/${tagId}`, {
                method: 'DELETE',
                headers: {
    ...
    featuredToggles.forEach(toggle => {
        toggle.addEventListener('change', async function() {
            ...
            try {
                ...
                const response = await fetch(`{{ url('admin/blog-tags') }}/${tagId}/toggle-featured`, {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': csrfToken ? csrfToken.content : '',
                        'Accept': 'application/json',
