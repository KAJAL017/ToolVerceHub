        ...
        <div class="mb-4">
            ...
            <select name="icon_emoji" id="iconSelect" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500">
                ...
                @foreach($icons as $icon)
                    <option value="{{ $icon->icon }}" {{ old('icon_emoji') == $icon->icon ? 'selected' : '' }}>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        .then(data => {
            if (data.success) {
                window.location.href = '{{ route("admin.blog-categories.index") }}?success=' + encodeURIComponent(data.message || 'Category created successfully!');
            } else if (data.errors) {
