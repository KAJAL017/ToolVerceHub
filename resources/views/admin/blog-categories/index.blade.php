        ...
        <table class="w-full" id="categoriesTable">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    ...
                    <th class="px-6 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition-colors" data-sort="blogs_count">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-center">
                        <div class="flex items-center justify-center">
                            @if($category->icon_emoji)
                                <span class="text-3xl">{{ $category->icon_emoji }}</span>
                            @else
                ...
                <tr>
                    <td colspan="7" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center justify-center">
                            ...
                            <p class="text-gray-400 text-sm mt-1 mb-4">Create your first category to organize your blogs</p>
                            <a href="{{ route('admin.blog-categories.create') }}" class="inline-flex items-center px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white font-semibold rounded-lg transition-colors">
                                <i class="fas fa-plus mr-2"></i>
                                Create Category
                            </a>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function handleDelete(categoryId, categoryName) {
        const confirmed = await showConfirmDialog({
            ...
            message: `Are you sure you want to delete "${categoryName}"? This will unlink all blogs from this category.`,
        ...
        if (confirmed) {
            ...
            fetch(`{{ url('admin/blog-categories') }}/${categoryId}`, {
                method: 'DELETE',
                headers: {
    ...
    featuredToggles.forEach(toggle => {
        toggle.addEventListener('change', async function() {
            ...
            try {
                ...
                const response = await fetch(`{{ url('admin/blog-categories') }}/${categoryId}/toggle-featured`, {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': csrfToken ? csrfToken.content : '',
                        'Accept': 'application/json',
