...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
    ...
    <div class="p-6 border-b border-gray-200">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h3 class="text-lg font-bold text-gray-800">All Tool Categories</h3>
                <p class="text-sm text-gray-500 mt-1">Manage your tool categories</p>
            </div>
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('admin.tool-categories.create') }}" class="inline-flex items-center px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white font-semibold rounded-lg transition-colors">
    ...
    <div class="overflow-x-auto">
        <table class="w-full" id="categoriesTable">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition-colors" data-name="{{ strtolower($category->name) }}" data-slug="{{ strtolower($category->slug) }}" data-description="{{ strtolower($category->description ?? '') }}">
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center">
                            <div class="flex-shrink-0 h-10 w-10 bg-gradient-to-br 
                                @if($category->color == 'g') from-green-400 to-green-600
                                @elseif($category->color == 'c') from-orange-400 to-orange-600
                                @elseif($category->color == 'b') from-blue-400 to-blue-600
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <code class="px-2 py-1 bg-gray-100 text-gray-800 rounded text-xs font-mono">{{ $category->slug }}</code>
                    </td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-center">
                        <div class="flex items-center justify-center gap-2">
                            <a href="{{ route('admin.tool-categories.edit', $category) }}" 
                ...
                <tr>
                    <td colspan="6" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center justify-center">
                            ...
                            <p class="text-gray-400 text-sm mt-1 mb-4">Create your first category to organize your tools</p>
                            <a href="{{ route('admin.tool-categories.create') }}" class="inline-flex items-center px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white font-semibold rounded-lg transition-colors">
                                <i class="fas fa-plus mr-2"></i>
                                Create Category
                            </a>
    ...
    <div class="px-6 py-4 border-t border-gray-200 bg-gray-50">
        <div class="flex items-center justify-between">
            <div class="text-sm text-gray-600">
                Showing <span id="showingCount" class="font-semibold text-gray-900">{{ $categories->count() }}</span> of <span class="font-semibold text-gray-900">{{ $categories->total() }}</span> categories
            </div>
            <div>
                {{ $categories->links() }}
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function sortTable(column, direction) {
        ...
        visibleRows.sort((a, b) => {
            ...
            if (column === 'name') {
                ...
                aValue = parseInt(a.querySelector('td:nth-child(4) span').textContent.trim());
                bValue = parseInt(b.querySelector('td:nth-child(4) span').textContent.trim());
            }
    ...
    searchInput.addEventListener('input', function() {
        ...
        allRows.forEach(row => {
            ...
            const matches = name.includes(searchTerm) || slug.includes(searchTerm) || description.includes(searchTerm);
    ...
    async function handleDelete(categoryId, categoryName) {
        const confirmed = await showConfirmDialog({
            ...
            message: `Are you sure you want to delete "${categoryName}"? This will unlink all tools from this category.`,
        ...
        if (confirmed) {
            fetch(`{{ url('admin/tool-categories') }}/${categoryId}`, {
                method: 'DELETE',
                headers: {
    ...
    featuredToggles.forEach(toggle => {
        toggle.addEventListener('change', async function() {
            ...
            try {
                ...
                const response = await fetch(`{{ url('admin/tool-categories') }}/${categoryId}/toggle-featured`, {
                    method: 'POST',
                    headers: {
                ...
                if (data.success) {
                    ...
                    toggleSwitch.checked = data.is_featured;
