...
@section('page-title', 'Edit Tool Category')
@section('page-subtitle', 'Update category details')

@section('content')

<form id="categoryForm" action="{{ route('admin.tool-categories.update', $toolCategory) }}" method="POST">
    @csrf
    @method('PUT')
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" id="name" value="{{ $toolCategory->name }}" required
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="slug" value="{{ $toolCategory->slug }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
                ...
                <div class="mb-4">
                    ...
                    <select name="icon" id="iconSelect" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500">
                        ...
                        @foreach($icons ?? [] as $icon)
                            <option value="{{ $icon->icon }}" {{ $toolCategory->icon == $icon->icon ? 'selected' : '' }}>{{ $icon->icon }} {{ $icon->name }} ({{ $icon->category }})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" rows="3"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
                        placeholder="Brief description of this category">{{ $toolCategory->description }}</textarea>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                ...
                <div class="mb-4">
                    ...
                    <select name="color" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500">
                        <option value="g" {{ $toolCategory->color == 'g' ? 'selected' : '' }}>Green</option>
                        <option value="c" {{ $toolCategory->color == 'c' ? 'selected' : '' }}>Coral</option>
                        <option value="b" {{ $toolCategory->color == 'b' ? 'selected' : '' }}>Blue</option>
                        <option value="a" {{ $toolCategory->color == 'a' ? 'selected' : '' }}>Amber</option>
                    </select>
                </div>
                
                <div class="mb-4">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Display Order</label>
                    <input type="number" name="display_order" value="{{ $toolCategory->display_order }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
                        placeholder="0">
                ...
                <div class="mb-4">
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" {{ $toolCategory->is_active ? 'checked' : '' }} class="w-4 h-4 text-primary-600 border-gray-300 rounded focus:ring-primary-500">
                ...
                <div class="mb-4">
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" name="is_featured" {{ $toolCategory->is_featured ? 'checked' : '' }} class="w-5 h-5 text-yellow-500 border-gray-300 rounded focus:ring-yellow-500 focus:ring-2">
                ...
                <div class="mb-4">
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" name="show_in_header" {{ $toolCategory->show_in_header ? 'checked' : '' }} class="w-5 h-5 text-blue-500 border-gray-300 rounded focus:ring-blue-500 focus:ring-2">
                        <span class="ml-3 text-sm font-semibold text-gray-700">
                ...
                <div class="mb-4">
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" name="show_in_home" {{ $toolCategory->show_in_home ? 'checked' : '' }} class="w-5 h-5 text-green-500 border-gray-300 rounded focus:ring-green-500 focus:ring-2">
                        <span class="ml-3 text-sm font-semibold text-gray-700">
                ...
                <div class="mb-4">
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" name="show_in_built" {{ $toolCategory->show_in_built ? 'checked' : '' }} class="w-5 h-5 text-purple-500 border-gray-300 rounded focus:ring-purple-500 focus:ring-2">
                        <span class="ml-3 text-sm font-semibold text-gray-700">
