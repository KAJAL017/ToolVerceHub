...
@section('page-subtitle', 'Update tool details')

@section('content')

<form id="toolForm" action="{{ route('admin.tools.update', $tool) }}" method="POST">
    @csrf
    @method('PUT')
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="slug" value="{{ $tool->slug }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
                ...
                <div class="mb-4">
                    ...
                    <select name="icon" id="iconSelect" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500">
                        ...
                        @foreach($icons as $icon)
                            <option value="{{ $icon->icon }}" {{ $tool->icon == $icon->icon ? 'selected' : '' }}>{{ $icon->icon }} {{ $icon->name }} ({{ $icon->category }})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" rows="3"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
                        placeholder="Brief description of this tool">{{ $tool->description }}</textarea>
                </div>
                
                <div class="mb-4">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">URL</label>
                    <input type="text" name="url" value="{{ $tool->url }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
                ...
                <div class="mb-4">
                    ...
                    <div class="custom-multi-select">
                        <select name="tool_tags[]" id="toolTagsSelect" multiple class="hidden">
                            @foreach($tags as $tag)
                                <option value="{{ $tag->id }}" {{ $tool->toolTags->contains($tag->id) ? 'selected' : '' }}>{{ $tag->name }}</option>
                            @endforeach
                        </select>
                        <div class="custom-select-trigger border border-gray-300 rounded-lg px-4 py-2 bg-white cursor-pointer relative" data-target="toolTagsSelect">
                            <div class="selected-tags-container flex flex-wrap gap-2 items-center min-h-[24px]" id="selectedTagsContainer">
                                @if($tool->toolTags->count() > 0)
                                    @foreach($tool->toolTags as $selectedTag)
                                        <span class="selected-tag" data-value="{{ $selectedTag->id }}">
                                            {{ $selectedTag->name }}
                                            <i class="fas fa-times" onclick="removeTagClick(this, '{{ $selectedTag->id }}', 'toolTagsSelect')"></i>
                                        </span>
                                    @endforeach
                                @else
                            ...
                            <div class="custom-select-options">
                                @foreach($tags as $tag)
                                    <div class="custom-select-option" data-value="{{ $tag->id }}">
                                        <input type="checkbox" class="mr-2" {{ $tool->toolTags->contains($tag->id) ? 'checked' : '' }}>
                                        <span>{{ $tag->name }}</span>
                                    </div>
                                @endforeach
                            </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="tool_count" value="{{ $tool->tool_count }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
                        placeholder="e.g., 80+">
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                ...
                <div class="mb-4">
                    ...
                    <select name="color" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500">
                        <option value="g" {{ $tool->color == 'g' ? 'selected' : '' }}>Green</option>
                        <option value="c" {{ $tool->color == 'c' ? 'selected' : '' }}>Coral</option>
                        <option value="b" {{ $tool->color == 'b' ? 'selected' : '' }}>Blue</option>
                        <option value="a" {{ $tool->color == 'a' ? 'selected' : '' }}>Amber</option>
                    </select>
                </div>
                ...
                <div class="mb-4">
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" name="is_featured" {{ $tool->is_featured ? 'checked' : '' }} class="w-5 h-5 text-yellow-500 border-gray-300 rounded focus:ring-yellow-500 focus:ring-2">
                ...
                <div class="mb-4">
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" name="is_free" {{ $tool->is_free ? 'checked' : '' }} class="w-5 h-5 text-green-500 border-gray-300 rounded focus:ring-green-500 focus:ring-2">
                        <span class="ml-3 text-sm font-semibold text-gray-700">
                ...
                <div class="mb-4">
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" name="show_in_hero" {{ $tool->show_in_hero ? 'checked' : '' }} class="w-5 h-5 text-purple-500 border-gray-300 rounded focus:ring-purple-500 focus:ring-2">
                        <span class="ml-3 text-sm font-semibold text-gray-700">
                ...
                <div class="mb-4">
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" {{ $tool->is_active ? 'checked' : '' }} class="w-5 h-5 text-primary-600 border-gray-300 rounded focus:ring-primary-500 focus:ring-2">
                        <span class="ml-3 text-sm font-semibold text-gray-700">
