...
@section('title', \App\Models\Setting::where('key', 'blog_page_title')->value('value') ?? 'Blog — Free Tool Guides, PDF Tips & Gaming News | ToolVerse Hub')
...
/* GRID & LIST POSTS */
.post-grid{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:28px}
.pc{background:#fff;border:1.5px solid var(--bdr);border-radius:var(--r20);overflow:hidden;box-shadow:var(--s1);display:flex;flex-direction:column;transition:all .22s;text-decoration:none;color:inherit}
...
.post-list{display:flex;flex-direction:column;gap:12px;margin-bottom:30px}
.ph{background:#fff;border:1.5px solid var(--bdr);border-radius:var(--r20);overflow:hidden;display:flex;box-shadow:var(--s1);transition:all .2s;text-decoration:none;color:inherit}
...
<div class="blog-hero">
  ...
  <div class="wrap bh-inner">
    <nav class="bc" aria-label="Breadcrumb"><a href="/">🏠 Home</a><span>›</span><span>Blog</span></nav>
    <div class="eyebrow">Resource Hub</div>
    <h1 class="h1" style="margin-bottom:12px">Tips, Guides &amp; <em style="color:var(--g)">Tutorials</em></h1>
    ...
    <div style="position:relative;max-width:460px">
      <div class="search-bar" role="search">
        <input type="text" id="srch" placeholder="Search articles… e.g. 'PNG to JPG'" aria-label="Search articles" autocomplete="off">
...
<div class="filter-bar">
  <div class="wrap"><div class="filter-inner" role="group" aria-label="Filter by category">
    @php
      $totalBlogs = \App\Models\Blog::where('status', 'published')->count();
    @endphp
    <a href="{{ route('blog') }}" class="ftab {{ !$selectedCategory ? 'on-all' : '' }}" data-cat="all">
      📚 All <span class="fcnt">{{ $totalBlogs }}</span>
    </a>
    @foreach($categories as $category)
      @if($category->blogs_count > 0)
        <a href="{{ route('blog', ['category' => $category->slug]) }}" 
          ...
          <span class="fcnt">{{ $category->blogs_count }}</span>
        </a>
      @endif
    @endforeach
  </div></div>
...
<div class="wrap">
  <div class="blog-layout">
    <main>
      @if($featuredBlog)
      <div class="feat-label">⭐ Featured Article</div>
      <a href="{{ route('blog.post', $featuredBlog->slug) }}" class="feat-card rv">
        @if($featuredBlog->featured_image)
          <img src="{{ asset('storage/' . $featuredBlog->featured_image) }}" alt="{{ $featuredBlog->title }}" class="fc-img" style="width:100%;min-height:240px;object-fit:cover">
        @else
          <div class="fc-img ft1">{{ $featuredBlog->featured_image_emoji ?? '📝' }}</div>
        @endif
        <div class="fc-body">
          <div class="post-meta">
            <span class="pcat pcg">{{ $featuredBlog->category->name ?? 'General' }}</span>
            <span>·</span>
            <time datetime="{{ $featuredBlog->published_date }}">{{ $featuredBlog->published_date->format('F d, Y') }}</time>
          </div>
          <h2 class="h3" style="margin-bottom:10px">{{ $featuredBlog->title }}</h2>
          <p>{{ $featuredBlog->excerpt }}</p>
          <div class="post-foot">
            <span class="post-rt">📖 {{ $featuredBlog->read_time ?? 5 }} min read</span>
            <span class="post-rm prmg">Read Full Guide →</span>
          </div>
        </div>
      </a>
      @endif

      <div class="sdiv">Latest Posts</div>
      <div class="post-grid" id="post-grid">
        @foreach($latestBlogs as $index => $blog)
        <a href="{{ route('blog.post', $blog->slug) }}" class="pc rv {{ $index % 2 == 1 ? 'd1' : '' }}">
          @if($blog->featured_image)
            <img src="{{ asset('storage/' . $blog->featured_image) }}" alt="{{ $blog->title }}" class="pc-img" style="width:100%;height:140px;object-fit:cover">
          @else
            <div class="pc-img pt{{ ($index % 4) + 1 }}">{{ $blog->featured_image_emoji ?? '📝' }}</div>
          @endif
          <div class="pc-body">
            <div class="post-meta" style="margin-bottom:6px">
              <span class="pcat pcg">{{ $blog->category->name ?? 'General' }}</span>
              <span>· {{ $blog->published_date->format('M d') }}</span>
            </div>
            <h3>{{ $blog->title }}</h3>
            <p>{{ Str::limit($blog->excerpt, 100) }}</p>
            <div class="pc-foot">
              <span class="post-rt">📖 {{ $blog->read_time ?? 5 }} min</span>
              <span class="post-rm prmg">Read →</span>
            </div>
          </div>
        </a>
        @endforeach
      ...
      <div class="no-results" id="no-results"><h3>No articles found</h3><p>Try a different search term or click "All" to reset.</p></div>

      <div class="sdiv">More Articles</div>
      <div class="post-list" id="post-list">
        @foreach($moreBlogs as $index => $blog)
        <a href="{{ route('blog.post', $blog->slug) }}" class="ph rv {{ $index % 2 == 1 ? 'd1' : '' }}">
          @if($blog->featured_image)
            <img src="{{ asset('storage/' . $blog->featured_image) }}" alt="{{ $blog->title }}" class="ph-img" style="width:106px;min-height:106px;object-fit:cover;flex-shrink:0">
          @else
            <div class="ph-img pi{{ ($index % 4) + 1 }}">{{ $blog->featured_image_emoji ?? '📝' }}</div>
          @endif
          <div class="ph-body">
            <div class="post-meta" style="margin-bottom:5px">
              <span class="pcat pcg">{{ $blog->category->name ?? 'General' }}</span>
              <span>· {{ $blog->published_date->format('M d') }} · � {{ $blog->read_time ?? 5 }} min</span>
            </div>
            <h3>{{ $blog->title }}</h3>
      ...
      @if($moreBlogs->hasPages())
      <div class="pagi" role="navigation" aria-label="Pagination">
        @if($moreBlogs->onFirstPage())
          <button class="pg" aria-label="Previous" disabled style="opacity:0.5;cursor:not-allowed">←</button>
        @else
          <a href="{{ $moreBlogs->previousPageUrl() }}" class="pg" aria-label="Previous">←</a>
        @endif
        
        @foreach($moreBlogs->getUrlRange(1, $moreBlogs->lastPage()) as $page => $url)
          @if($page == $moreBlogs->currentPage())
        ...
        @if($moreBlogs->hasMorePages())
          <a href="{{ $moreBlogs->nextPageUrl() }}" class="pg" aria-label="Next">→</a>
        @else
    ...
    <aside class="sidebar">
      <div class="sw">
        ...
        <div class="sp @if($tool->color == 'c') sc @elseif($tool->color == 'b') sb @endif">
          <div class="sp-top">
            <span class="sp-ic">{{ $tool->icon ?? '🔧' }}</span>
            <div>
              <div class="sp-nm">{{ $tool->name }}</div>
              @if($tool->tool_count)
              <div class="sp-ct">{{ $tool->tool_count }}</div>
              @endif
            </div>
          </div>
          @if($tool->description)
          ...
          @endif
          @if($tool->url)
          <a href="{{ $tool->url }}" class="sp-lk g" target="_blank" rel="noopener">Try Free →</a>
          @endif
        </div>
        @empty
        <div class="sp">
          ...
          @forelse($sidebarCategories as $category)
            @if($category->blogs_count > 0)
              <a href="{{ route('blog', ['category' => $category->slug]) }}" 
                ...
                {{ $category->icon_emoji ?? '📁' }} {{ $category->name }}
                <span class="cl-cnt">{{ $category->blogs_count }}</span>
              </a>
            @endif
          @empty
        ...
        <div class="tags-cloud">
          @forelse($popularTags as $tag)
            <a href="{{ route('blog', ['tag' => $tag->slug]) }}" 
               class="tag-btn {{ $selectedTag == $tag->slug ? 'active' : '' }}" 
    ...
    function performSearch(query) {
        ...
        fetch(searchUrl)
            ...
            .then(data => {
                ...
                if (data.length === 0) {
                    searchResults.innerHTML = '<div class="search-no-results">No articles found. Try different keywords.</div>';
                } else {
                    let html = '';
                    data.forEach(blog => {
                        html += '<a href="' + blog.url + '" class="search-result-item">';
                        html += '<div class="search-result-category">' + blog.category_icon + ' ' + (blog.category || 'Uncategorized') + '</div>';
                        html += '<div class="search-result-title">' + highlightText(blog.title, query) + '</div>';
                        html += '<div class="search-result-excerpt">' + blog.excerpt + '</div>';
                        html += '</a>';
                    });
...
@section('schema')
@include('website.partials.blog-listing-schema')
@endsection
